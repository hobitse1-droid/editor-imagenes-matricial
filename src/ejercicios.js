...
// ============================================
// Nombre del estudiante: Leonardo Torres Tellez
// Fecha: 18/11/2025
// Grupo: B
...
function obtenerDimensionesImagen(rutaImagen) {
  ...
  // 3. Retornar objeto con las dimensiones
...
function ajustarBrillo(matriz, factor) {
  ...
  // 2. Recorrer cada píxel y ajustar brillo
  for (let i = 0; i < resultado.length; i++) {
    for (let j = 0; j < resultado[i].length; j++) {
...
function convertirEscalaGrises(matriz) {
  ...
  // 2. Recorrer cada píxel y calcular el valor de gris
  for (let i = 0; i < resultado.length; i++) {
    for (let j = 0; j < resultado[i].length; j++) {
      const pixel = matriz[i][j];
      const gris = limitarValorColor(
        0.299 * pixel.r + 0.587 * pixel.g + 0.114 * pixel.b
      );

      // 3. Asignar el valor de gris a los tres canales
      resultado[i][j] = {
        r: gris,
        g: gris,
        b: gris,
...
function voltearHorizontal(matriz) {
  ...
  // 2. Recorrer cada fila y aplicar reverse()
  for (let i = 0; i < resultado.length; i++) {
...
function mezclarImagenes(matriz1, matriz2, factor) {
  ...
  // 3. Para cada pixel, aplicar la combinación lineal
  for (let i = 0; i < resultado.length; i++) {
    for (let j = 0; j < resultado[i].length; j++) {
...
function aplicarSepia(matriz) {
  ...
  // 2. Recorrer cada píxel y aplicar la transformación sepia
  for (let i = 0; i < resultado.length; i++) {
    for (let j = 0; j < resultado[i].length; j++) {
      const pixel = matriz[i][j];

      const nuevoR = limitarValorColor(0.393 * pixel.r + 0.769 * pixel.g + 0.189 * pixel.b);
      const nuevoG = limitarValorColor(0.349 * pixel.r + 0.686 * pixel.g + 0.168 * pixel.b);
      const nuevoB = limitarValorColor(0.272 * pixel.r + 0.534 * pixel.g + 0.131 * pixel.b);

      resultado[i][j] = {
        r: nuevoR,
        g: nuevoG,
        b: nuevoB,
        a: pixel.a // alpha se mantiene igual
      };
    }
  }
...
function detectarBordes(matriz, umbral = 50) {
  ...
  // 3. Recorrer cada píxel (excepto última fila y última columna)
  for (let i = 0; i < grises.length; i++) {
    for (let j = 0; j < grises[i].length; j++) {
      const pixel = grises[i][j];
      const valor = pixel.r; // ya está en gris, basta con usar r

      let diferencia = 0;

      // Comparar con el vecino derecho
      if (j + 1 < grises[i].length) {
        diferencia = Math.max(diferencia, Math.abs(valor - grises[i][j + 1].r));
      }

      // Comparar con el vecino inferior
      if (i + 1 < grises.length) {
        diferencia = Math.max(diferencia, Math.abs(valor - grises[i + 1][j].r));
      }

      // 4. Decidir si es borde o no
      const color = diferencia > umbral ? 255 : 0;
